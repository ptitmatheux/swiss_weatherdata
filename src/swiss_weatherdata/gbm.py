...
def get_stations_info(full_description=False, network='smn'):
    """ 
    ...
    ARGUMENTS: 
        ...
        - network (string): selects which meteorological station network to use; one of the following: 
                            'smn' (default): the SwissMetNet automatic measurement network ; see
                                             https://www.meteoswiss.admin.ch/weather/measurement-systems/land-based-stations/automatic-measurement-network.html         
                            'nbcn': the Swiss National Basic Climatological Network ; see
    ...
    URL = f"https://data.geo.admin.ch/ch.meteoschweiz.ogd-{network}/ogd-{network}_meta_stations.csv"
...
def _get_station_name(sta, network='smn'):
    """ 
    Get the name of a SMN/NBCN station given its station_abbr.

    ARGUMENTS: 
        - sta (string): the SMN/NBCN meteorological station 3-letters abbreviation (e.g. 'GVE', 'CGI', 'PAY',...)
        - network (string): selects which meteorological station network to use; one of the following: 
                    'smn' (default): the SwissMetNet automatic measurement network       
                    'nbcn': the Swiss National Basic Climatological Network
    ...
    if len(df_sta) == 1:
        sta_name = df_sta['station_name'].item()
    else:
        raise ValueError(f"There is no station with nat_abbr '{sta}' in the requested network !")
    return sta_name


def get_meteo_parameters_info(full_description=False, lookup=None, language='fr', network='smn'):
    """ 
    ...
    URL = f"https://data.geo.admin.ch/ch.meteoschweiz.ogd-{network}/ogd-{network}_meta_parameters.csv"
...
def get_parameters_by_station(sta, full_description=False, lookup=None, language='fr', network='smn'):
    """ 
    Get information about all available meteorological parameters for a given meteorological station.

    ARGUMENTS:
        - sta (string): the SMN/NBCN meteorological station (e.g. 'GVE', 'CGI', 'PAY',...)
        - full_description (boolean, default=False): if True, then all available metadata is returned ; by default
                                                     a selection of the most relevant metadata is returned
        - lookup (string, optional): allows to filter according to a group of parameters (temperature, precipitation, wind...)
                                     only parameters that contain the lookup string are then returned. The lookup is case-insensitive,
    ...
    URL = f"https://data.geo.admin.ch/ch.meteoschweiz.ogd-{network}/ogd-{network}_meta_datainventory.csv"
    ...
    if len(sta_params) == 0:
        raise ValueError(f"No parameter found for station {sta} ; check out the station and the network !")
    ...
    output = pd.merge(
        sta_params,
        get_meteo_parameters_info(full_description, lookup, language, network),
        how='inner',
        on='parameter_shortname')
    return output


def get_param_description(shortname, language='fr', network='smn'):
    """ 
    ...
    all_param_info = get_meteo_parameters_info(full_description=False, language=language, network=network)
...
def check_parameter_availability(shortname, sta, network='smn'):
    """ 
    ...
    sta_params = get_parameters_by_station(sta, network=network)
...
def _get_nbcn_data(sta, granularity, past_type=None):
    """
    Get data from a remote csv file into a pd.DataFrame

    ARGUMENTS:
        - sta (string): nat_abbr_tx abbreviation
        - granularity (string): daily - 'd' ; monthly - 'm' ; yearly - 'y'
        - past_type (string): one of 'recent' or 'historical'
    ...
    if granularity == 'd' and past_type is None:
        raise ValueError(f"For granularity '{granularity}', 'past_type' must be provided !")
    if granularity in ['m', 'y'] and past_type is not None:
        print(f"For granularities 'm' (monthly) and 'y' (yearly), argument 'past_type' is useless and ignored.")

    # the basename of the file associated with given location and time granularity
    url_suffix_base = f"ogd-nbcn_{sta.lower()}_{granularity}"
    ...
    url = os.path.join(BASE_URL_NBCN, sta.lower(), url_suffix + '.csv')
    #print(url)
...
def get_smn_measures(sta, parameters, beg=None, end=None, description_lang='fr'):
    """
    ARGUMENTS:
        - sta (string): the SMN (SwissMetNet) meteorological station (e.g. 'GVE', 'CGI', 'PAY',...)
                        check for available stations: get_stations_info(network='smn')
    ...
    for p in parameters:
        if check_parameter_availability(p, sta, network='smn'):
            pass
        else:
            raise ValueError(f"Parameter {p} is misspelled or not available for the SMN network !")
    ...
    df_params_by_sta = get_parameters_by_station(sta)
    param_granu_list = []
...
def get_nbcn_measures(sta, parameters, beg=None, end=None, description_lang='fr'):
    """
    ARGUMENTS:
        - sta (string): the NBCN (National Basic Climatological Network) meteorological station (e.g. 'GVE', 'PAY',...) ; 
                        check for available stations: get_stations_info(network='nbcn')
        - parameters (list): the requested meteorological parameters -- a list of strings corresponding 
                             to the parameter_shortname as given by get_parameters_by_station(sta)
        - beg/end (datetime or string, default=None): the start/end of the measurement period -- datetime object or string in format "AAAAMMDDHHmm".
                                                      If 'beg'=None, then all available data from the beginning of the recordings is retrieved.
    ...
    if beg is None and end is None:
        ...
        if beg is None:
            print(f"Retrieving measurements at {sta} ({_get_station_name(sta, network='nbcn')}) from the beginning of measurement to {end.strftime('%d-%m-%Y %H:%M')}")
        elif end is None:
            print(f"Trying to retrieve measurements at {sta} ({_get_station_name(sta, network='nbcn')}) from {beg.strftime('%d-%m-%Y %H:%M')} to latest available timestamp")
        else:
            print(f"Trying to retrieve measurements at {sta} ({_get_station_name(sta, network='nbcn')}) from {beg.strftime('%d-%m-%Y %H:%M')} to {end.strftime('%d-%m-%Y %H:%M')}")


    for p in parameters:
        if check_parameter_availability(p, sta, network='nbcn'):
            pass
        else:
            raise ValueError(f"Parameter {p} is misspelled or not available for the NBCN network !")
    ...
    # get and print out info about requested parameters:
    df_params_by_sta = get_parameters_by_station(sta, network='nbcn')
    param_granu_list = []
    is_any_data_available = False
    for param in parameters:
        param_row = df_params_by_sta.loc[df_params_by_sta['parameter_shortname'] == param]
    ...
    # retrieve all needed files containing the data and concatenate them:        
    if param_granu in ['m', 'y']: # for these granularities, there is a single file with all the data
        df = _get_nbcn_data(sta, param_granu)
    else:
        today_start = datetime.combine(date.today(), time())
        df_list = []
        if beg is None or beg.year < today_start.year: # if historical measures are requested
            df_list.append(_get_nbcn_data(sta, param_granu, 'historical'))

            if end is None:
                df_list.append(_get_nbcn_data(sta, param_granu, 'recent'))
